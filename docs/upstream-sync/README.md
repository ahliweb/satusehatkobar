...
- `LAST_AWCMS_MICRO_FETCH.md`: exact upstream AWCMS-Micro commit copied into `awcms-latest/`
...
3. Read `LAST_AWCMS_MICRO_FETCH.md` for the exact upstream AWCMS-Micro reference revision.
4. Read `LAST_VALIDATION.md` for the latest validation result.
5. Read `COMPATIBILITY_MATRIX.md` for feature-level adoption and adaptation notes.
6. Read `DIVERGENCE_LOG.md` for downstream decisions that intentionally differ from a plain upstream checkout.
7. Read `ISSUE_CLASSIFICATION_DOWNSTREAM_VS_UPSTREAM.md` before deciding whether a finding belongs in downstream boundaries or upstream EmDash core.
8. Read `UPSTREAM_PR_PLAN_ADMIN_SIDEBAR_ORDERING.md` before preparing the upstream sidebar ordering PR plan.
