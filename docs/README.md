...
- `prd/`: Satu Sehat Kobar product specification package (26 Markdown documents: 1 PRD induk + 25 supporting docs). Entry point: `prd/20.Master Document Index and Implementation Guide.docx.md`; PRD-to-workspace bridge: `prd/24.TECHNICAL_IMPLEMENTATION_REFERENCES.md`; AI-ready issue playbook + backlog coverage index: `prd/25.AI_READY_ISSUE_PLAYBOOK_AND_INDEX.md`
...
- `environment-configuration.md`: canonical environment model, the intentional split between `awcms-sskobar-cloudflare` (template path/package) and `awcms-sskkobar-*` (deployment/resource values), plus local/prod sync rules and GitHub/Cloudflare mapping
...
- `upstream-sync/LAST_AWCMS_MICRO_FETCH.md`: exact upstream AWCMS-Micro revision copied into `awcms-latest/`
...
- `deployment/`: deployment runbooks for Cloudflare, D1, R2, Workers/Pages, and the deployment checklist
- `security/`: security, privacy, audit, backup/restore, ISO, and Indonesia compliance baseline documentation
