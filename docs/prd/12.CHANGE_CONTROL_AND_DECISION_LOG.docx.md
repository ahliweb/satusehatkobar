...
### DEC-019: Default Persistensi = D1 Langsung (`ctx.db`) untuk Semua Plugin SSK
...
| **Tanggal**| 2026-06-16 |
| **Requestor** | Tech Lead / PO |
| **Keputusan** | Mekanisme persistensi fisik **default untuk seluruh plugin Satu Sehat Kobar adalah D1 langsung** melalui `ctx.db` (query builder Kysely di atas binding D1 `DB`), bukan EmDash plugin storage collections. Skema relasional di `04.DATABASE_MVP_SCHEMA.docx.md` menjadi skema fisik yang diimplementasikan apa adanya (tabel, kolom, relasi, index). Karena EmDash tidak menyediakan plugin migration runner (peta migrasi core bersifat frozen), **setiap plugin memiliki dan mengelola lifecycle skema-nya sendiri**: DDL idempoten (`CREATE TABLE IF NOT EXISTS`, `CREATE INDEX IF NOT EXISTS`, dan migrasi alter terurut) dijalankan via `ctx.db` pada lifecycle hook `install`/`activate`, ditandai dengan tabel migrasi milik plugin (mis. `<prefix>_migrations`). `ctx.kv` tetap dipakai untuk cache (mis. dashboard 15 menit per DEC-014); `ctx.storage` collections **tidak** dipakai sebagai store domain default. |
| **Rasional** | (1) Kemudahan pengembangan: developer bekerja dengan skema relasional eksplisit dan SQL/Kysely standar yang sama persis dengan Doc 04, bukan abstraksi key-value collection. (2) Kemudahan integrasi berikutnya: Phase 2 (SIMPEG, SIPD, BSrE, BI/pelaporan, ekspor data) jauh lebih mudah di atas tabel relasional nyata dengan join, FK, dan index daripada di atas storage collections. (3) Konsistensi: semua plugin memakai satu model persistensi sehingga query, audit, backup, dan migrasi seragam. Trade-off yang diterima: plugin wajib mengelola migrasi skema-nya sendiri secara idempoten dan tidak mendapat provisioning/index otomatis dari storage collections. |
| **Dampak DB** | Implementasi tabel mengikuti Doc 04 secara langsung di D1. Setiap plugin menambah modul migrasi internal idempoten + tabel `<prefix>_migrations`. Relasi lintas plugin tetap di-enforce di service layer + snapshot (bukan FK runtime rapuh) sesuai DEC-003/arsitektur plugin. |
| **Dampak Dokumen** | `24.TECHNICAL_IMPLEMENTATION_REFERENCES.md` (§4.4, §7.1, §7.2), `20.Master Document Index` (glosarium), `04.DATABASE_MVP_SCHEMA` (catatan model persistensi), serta root `README.md`, `AGENTS.md`, dan docs root terkait diselaraskan. |
| **Catatan** | Memakai EmDash plugin storage collections atau `ctx.kv` sebagai store domain untuk kasus khusus tetap boleh, tetapi merupakan **pengecualian** yang harus dicatat sebagai keputusan tersendiri. Ini membalik default sementara yang tercantum di Doc 24 v1.1; Doc 24 di-bump untuk menyatakan D1 langsung sebagai default. |
| **Status** | Aktif |
...
| CHG-009 | Tetapkan default persistensi plugin = D1 langsung (`ctx.db`), bukan storage collections | Kemudahan pengembangan dan integrasi Phase 2 (SIMPEG/SIPD/BI) | Plugin kelola migrasi idempoten sendiri; Doc 04 jadi skema fisik | Disetujui — masuk MVP (DEC-019) | Diimplementasikan | 2026-06-16 |
