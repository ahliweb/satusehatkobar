...
| 24 | `24.TECHNICAL_IMPLEMENTATION_REFERENCES.md` | v1.1 | Active | Referensi teknis implementasi: runtime AWCMS-Micro/EmDash (format plugin Native, penamaan paket, persistensi `ctx.storage`/`ctx.kv`, binding Cloudflare), mapping PRD ke workspace, boundary, plugin owner, deployment, security | Sebelum membuat issue teknis atau coding |
...
| KV Cache | Cache dashboard via `ctx.kv` (TTL 15 menit) — bukan binding KV terpisah; template hanya membind KV `SESSION` | PRD, `03.Plugin Architecture`, `24.Technical Implementation References` |
...
| Storage Collections | Mekanisme persistensi plugin EmDash (`ctx.storage.<collection>`) yang dipakai sebagai implementasi fisik default schema logis Doc 04 | `24.Technical Implementation References`, `04.DB Schema` |
...
|---------------------|-------------------|--------|
| `04.DB Schema` | `03.Plugin Architecture` + `24.Technical Implementation References` (§4.4) | Salah membuat tabel tanpa prefix, coupling antar plugin, atau menulis SQL migration manual padahal runtime memakai `ctx.storage`/`ctx.kv` |
