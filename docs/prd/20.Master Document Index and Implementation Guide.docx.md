...
Dokumen ini adalah **indeks master dan panduan navigasi** seluruh paket dokumentasi PRD Satu Sehat Kobar v1.5, yang terdiri dari **26 dokumen markdown**: 1 PRD induk dan 25 dokumen pendukung. Sebelum membaca dokumen teknis manapun, baca dokumen ini terlebih dahulu.
...
| 24 | `24.TECHNICAL_IMPLEMENTATION_REFERENCES.md` | v1.2 | Active | Referensi teknis implementasi: runtime AWCMS-Micro/EmDash (format plugin Native, penamaan paket, persistensi default D1 langsung via `ctx.db`, binding Cloudflare), mapping PRD ke workspace, boundary, plugin owner, deployment, security | Sebelum membuat issue teknis atau coding |
| 25 | `25.AI_READY_ISSUE_PLAYBOOK_AND_INDEX.md` | v1.0 | Active | Standar penulisan GitHub issue self-contained untuk AI model junior bertoken terbatas + indeks cakupan seluruh backlog (item → issue), skill eksekusi `.opencode/skills/sskobar-*` | Saat membuat/mengeksekusi issue implementasi |
...
| KV Cache | Cache dashboard via `ctx.kv` (TTL 15 menit) — bukan binding KV terpisah dan bukan store domain; template hanya membind KV `SESSION` | PRD, `03.Plugin Architecture`, `24.Technical Implementation References` |
| Plugin Native | Format plugin EmDash dengan admin React + akses data langsung, didaftarkan di `plugins: []`; format wajib untuk plugin MVP SSK | `24.Technical Implementation References`, `awcmsmicro-dev/skills/creating-plugins` |
| Direct D1 (`ctx.db`) | **Default persistensi** semua plugin SSK (DEC-019): tabel relasional langsung di D1 via `ctx.db` (Kysely); Doc 04 = skema fisik; tiap plugin kelola migrasi idempoten `<prefix>_migrations` sendiri | `24.Technical Implementation References` §4.4, `04.DB Schema`, `12.Change Control` DEC-019 |
| Storage Collections | Mekanisme persistensi plugin EmDash (`ctx.storage.<collection>`); **pengecualian** sejak DEC-019 (bukan default), harus dicatat di Doc 12 | `24.Technical Implementation References`, `04.DB Schema` |
| Nama Paket Plugin | Konvensi: direktori `awcms-micro-<key>`, npm `@awcms-micro/plugin-<key>`, slug `awcms-micro-<key>` | `03.Plugin Architecture`, `24.Technical Implementation References` |
...
|---------------------|-------------------|--------|
| `04.DB Schema` | `03.Plugin Architecture` + `24.Technical Implementation References` (§4.4) | Salah membuat tabel tanpa prefix, coupling antar plugin, atau lupa modul migrasi idempoten milik plugin (default D1 langsung via `ctx.db`, DEC-019) |
