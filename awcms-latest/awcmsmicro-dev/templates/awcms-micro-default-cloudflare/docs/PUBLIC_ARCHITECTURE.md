...
```mermaid
flowchart TD
  Admin[EmDash Admin] -->|edits| D1[(Cloudflare D1 / SQLite)]
  D1 --> Posts[posts]
  D1 --> News[news]
  D1 --> Pages[pages]
...
- `src/styles/public.css` defines the section + card utilities (`.section-shell`, `.section-heading`, `.service-card`, `.benefit-item`, `.testimonial-card`, `.faq-item`, `.profile-card`, hero motif), themed to the existing EmDash CSS tokens so light/dark theming is inherited from `Base.astro`.
...
- `Base.astro` keeps the existing `.reveal*`, `.card`, and `.btn*` utilities; `public.css` does not duplicate them.
...
sequenceDiagram
  ...
  participant MM as Mermaid initializer (Base.astro)
...
EmDash renders a PortableText `code` block (`language: "mermaid"`) as `<pre class="language-mermaid"><code class="language-mermaid">…`. The client-side initializer in `Base.astro` detects these and renders them with Mermaid 11, themed for light/dark. Service content seeded from ahliwebcom keeps its workflow/architecture diagrams.
