...
To support this without modifying EmDash's core package (which would block seamless upstream synchronization), AWCMS-Micro keeps the compatibility layer inside the SIKESRA plugin boundary and exposes it through `@awcms-micro/plugin-sikesra/navigation`.
